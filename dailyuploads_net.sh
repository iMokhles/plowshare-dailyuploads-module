...
MODULE_DAILYUPLOADS_NET_REGEXP_URL='https\?://\(www\.\)\?dailyuploads.net/'
MODULE_DAILYUPLOADS_NET_DOWNLOAD_OPTIONS="
AUTH,a,auth,a=USER:PASSWORD,User account"
MODULE_DAILYUPLOADS_NET_DOWNLOAD_RESUME=no
MODULE_DAILYUPLOADS_NET_DOWNLOAD_FINAL_LINK_NEEDS_COOKIE=yes
MODULE_DAILYUPLOADS_NET_DOWNLOAD_SUCCESSIVE_INTERVAL=
MODULE_DAILYUPLOADS_NET_PROBE_OPTIONS=""

dailyuploads_net_login() {
    local AUTH=$1
    local COOKIE_FILE=$2
    local BASE_URL=$3
...
dailyuploads_net_download() {
	local COOKIE_FILE=$1
	local URL=$2
    local BASE_URL="https://dailyuploads.net/"
    ...
    if [ -n "$AUTH" ]; then
        dailyuploads_net_login "$AUTH" "$COOKIE_FILE" "$BASE_URL" || return
